...
def prepare_dataloaders(
    ...
    train_loader = DataLoader(
        train_dataset, batch_size=config["data"]["batch_size_train"], shuffle=False, pin_memory=True, num_workers=10
    )

    val_loader = DataLoader(
        test_dataset, batch_size=config["data"]["batch_size_test"], shuffle=False, pin_memory=True, num_workers=10
    )
...
def main(config):
    ...
    train_loader, val_loader = prepare_dataloaders(dataset, config)
...
if __name__ == "__main__":
    ...
    config = {
        ...
        "model" : {
            "name" : "timm_vit_tiny16", # mlp_synth_dim-10_classes-7
            "model_seed" : 1,
                   },
    ...
    results_file = "synth_results/imagenet_t/all_results.csv" # "synth_results/resnet3072_test/all_results.csv"  
    ...
    #     # for temperature in [1.1 + 0.05 * i for i in range(20)]:
    #     for temperature in [0.6, 1, 2, 5]:
