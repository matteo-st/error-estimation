...
def main(config):
    ...
    # Method
        ...
        if hasattr(method.method, "fit"):
            ...
            train_preds, train_targets, train_scores = method.fit(train_dataloader, val_dataloader, name_save_file = f"train_fold{fold_idx}")
            evaluate_classification(train_preds, train_targets, train_scores, config.results_folder, name_save_file = f"train_fold{fold_idx}")   
        

        print(3 * "---", "Testing method", 3 * "---")
        test_preds, test_targets, test_scores = method.evaluate(test_dataloader, name_save_file = f"test_fold{fold_idx}")
        evaluate_classification(test_preds, test_targets, test_scores, config.results_folder, name_save_file = f"test_fold{fold_idx}")
